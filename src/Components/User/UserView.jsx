...
const navigate = useNavigate()
    ...
    const { previewData,userData,travellingDate,requiredTime,destinationAddress } = location.state || {};
...
value={`TN${dispatchData.serialNumber},DISP${dispatchData.dispatchNumber},${queryData.minecode},${
 travellingDate
      ? new Date(travellingDate).toLocaleString('en-GB', {
        ...
        .replace(',', '')    
      : '-'
},${dispatchData.totalDistance}Kms,${travellingDate && requiredTime ? (() => {
    ...
    })() : '-'} ,${data.mineralName}(${dispatchData.quantity}MT),${dispatchData.vehicleNo},${destinationAddress}`} size={49}
      level="H"
      fgColor='#000000'
    />
          </div>
    ...
    {travellingDate && requiredTime ? (() => {
      ...
      return` ${diffHours} hrs (${formatted})`;
          ...
          <tr>
            ...
            <td>{dispatchData.quantity}</td>
            <td>Driver Name:</td>
            <td>{dispatchData.driverName}</td>
          </tr>
          <tr>
            <td>Driver License No:</td>
            <td>{dispatchData.driverLicenseNo}</td>
            <td>Via</td>
            <td>{dispatchData.via}</td>
          </tr>
          <tr>
            <td>Driver Phone No :</td>
            <td>{dispatchData.driverPhoneNo}</td>
